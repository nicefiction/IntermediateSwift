...
/**
 ...
 so `monday` is `2` , `tuesda`y is `3` , and so on .
...
let wednesday = Day.init(rawValue : 4) // OPTION click 👉 init?(rawValue: Int)
...
class Human {
    ...
    throws {
        ...
        guard
            let _name = dictionary["name"] as? String ,
            let _age = dictionary["age"] as? Int
        else { throw HumanError.invalidData }
...
/**
 ...
 you'll notice that an `init()` method can be marked as both _throwing_ and _failable_ ,
